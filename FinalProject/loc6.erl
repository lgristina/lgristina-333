...
locationLoop() ->
   receive
      ...
      {_FromNode, search, GameClientNode, GameClientState}  ->
         io:fwrite("~sA gameClient on ~w is searching loc6.~n",[?id, GameClientNode]),
...
%--------
describe() ->
   io:fwrite("(6) You wake up disoriented in New York in 1975. You are at the second stop before you get to head back to the present ~n", []),
   io:fwrite("(6) You need to find the person with the code fragment! [search 1975]. ~n", []),
   io:fwrite("(6) Or if you have already collected the code fragments, move further into the future [go 1995]. ~n", []).


narrative() ->
   io:fwrite("(6) You go out to look for the person with the code fragment... ~n", []),
   io:fwrite("(6) You head into the bar you were instructed to go to and find someone you would not have expected~n", []),
   io:fwrite("(6) The person who holds the code tells you that it is 'a' ~n", []).