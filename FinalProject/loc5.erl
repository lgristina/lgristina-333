...
start(ServerNode) ->
   ...
   {gameServer, ServerNode} ! {node(), registerNewLocation, loc5},
...
locationLoop() ->
   receive
      ...
      {_FromNode, search, GameClientNode, GameClientState}  ->
         io:fwrite("~sA gameClient on ~w is searching loc5.~n",[?id, GameClientNode]),
...
%--------
describe() ->
   io:fwrite("(5) You wake up disoriented in Chicago in 1985. You are at the last stop before you get to head back to the present ~n", []),
   io:fwrite("(5) You need to find the person with the code fragment [search 1985]. ~n", []),
   io:fwrite("(5) Or if you have collected all of the code fragments, move on to the code room in the present [go 2025]. ~n", []).


narrative() ->
   io:fwrite("(5) You go out to look for the person with the code fragment... ~n", []),
   io:fwrite("(5) You head into the bar you were instructed to go to and find someone you would not have expected~n", []),
   io:fwrite("(5) The person who holds the code tells you that it is 'c' ~n", []).